    ...
    jQuery(function($){
        ...
        $('#media-encoder-run-preview').on('click', function(){
            ...
            $.post(ajaxurl, { 
                ...
                if(!res || !res.success){ 
                    $('#media-encoder-preview-result').text(res && res.data ? res.data : '預覽失敗'); 
                    return; 
                }
                const d = res.data;
        ...
        $('#media-encoder-bulk-start').on('click', function(){
            ...
            function step(){
                $.post(ajaxurl, { 
                    ...
                    if(!res || !res.success){ 
                        $('#media-encoder-bulk-status').text(res && res.data ? res.data : '批次失敗'); 
                        return; 
                    }
...
function media_encoder_convert_file_to_webp($src_path, $quality = 82) {
    ...
    if (class_exists('Imagick')) {
        try {
            ...
            error_log('Imagick WebP 轉換錯誤: ' . $e->getMessage() . ' - 檔案: ' . $src_path);
...
function media_encoder_convert_on_upload($metadata, $attachment_id) {
    ...
    if (is_wp_error($res)) {
        error_log('WebP 轉換失敗 (原圖): ' . $res->get_error_message() . ' - 檔案: ' . $file);
    } else if (file_exists($res['path'])) {
        // 僅生成 .webp 並保留原檔；更新顯示用的檔案大小資訊
        $metadata['filesize'] = filesize($res['path']);
    }
    ...
    if (!empty($metadata['sizes']) && is_array($metadata['sizes'])) {
        ...
        foreach ($metadata['sizes'] as $size_key => $size_info) {
            ...
            if (file_exists($size_path)) {
                ...
                if (is_wp_error($r)) {
                    error_log('WebP 轉換失敗 (尺寸 ' . $size_key . '): ' . $r->get_error_message() . ' - 檔案: ' . $size_path);
                } else if (file_exists($r['path'])) {
                    $metadata['sizes'][$size_key]['filesize'] = filesize($r['path']);
                }
            }
        }
...
function media_encoder_ajax_preview() {
    ...
    // 複製到預覽路徑，避免覆蓋正式 webp
    if (!@copy($r['path'], $preview_path)) {
        wp_send_json_error('預覽文件生成失敗');
    }
    
    $orig_size = filesize($file);
    $webp_size = filesize($preview_path);
...
function media_encoder_ajax_bulk() {
    ...
    foreach ($q->posts as $id) {
        ...
        if (is_wp_error($r)) { 
            error_log('批次 WebP 轉換失敗: ' . $r->get_error_message() . ' - 檔案: ' . $file);
            $skipped++; 
            continue; 
        }
            ...
            foreach ($meta['sizes'] as $k => $info) {
                ...
                if (file_exists($size_path)) {
                    ...
                    if (!is_wp_error($rr) && file_exists($rr['path'])) {
                        $meta['sizes'][$k]['filesize'] = filesize($rr['path']);
                    }
                }
            }
        }
...
function media_encoder_maybe_register_front_filters() {
    $settings = media_encoder_get_settings();
    if ($settings['enabled'] !== 'on') return;
    
    add_filter('post_thumbnail_html', 'media_encoder_filter_image_html', 10, 3);
    
    add_filter('get_avatar', function($avatar, $id_or_email, $size, $default, $alt){
        // 嘗試將 img 包成 picture，僅處理本站媒體庫的 URL
        if (strpos($avatar, 'src=') === false) return $avatar;
        if (!preg_match('/src=\"([^\"]+)\"/', $avatar, $m)) return $avatar;
        
        $orig = $m[1];
        if (!preg_match('/\.(jpe?g|png)$/i', $orig)) return $avatar;
        
        // 檢查是否為本站上傳的檔案
        $upload_dir = wp_upload_dir();
        if (strpos($orig, $upload_dir['baseurl']) !== 0) return $avatar;
        
        $webp = preg_replace('/\.(jpe?g|png)$/i', '.webp', $orig);
        $webp_path = str_replace($upload_dir['baseurl'], $upload_dir['basedir'], $webp);
        
        if (!file_exists($webp_path)) return $avatar;
    ...
    add_filter('the_content', function($content){
        ...
        return preg_replace_callback('/<img[^>]+src=\"([^\"]+)\"[^>]*>/i', function($m) use ($upload_url, $upload_dir){
            $img = $m[0]; 
            $src = $m[1];
            
            if (stripos($img, '<picture') !== false) return $img;
            if (strpos($src, $upload_url) !== 0) return $img;
            if (!preg_match('/\.(jpe?g|png)$/i', $src)) return $img;
            
            $webp = preg_replace('/\.(jpe?g|png)$/i', '.webp', $src);
            $webp_path = str_replace($upload_dir['baseurl'], $upload_dir['basedir'], $webp);
            
            if (!file_exists($webp_path)) return $img;
            
            return '<picture><source srcset="' . esc_url($webp) . '" type="image/webp">' . $img . '</picture>';
        }, $content);
    }, 20);
}
add_action('init', 'media_encoder_maybe_register_front_filters');
